import subprocess
from time import sleep
from common_lib import *

#
app_name = 'java'


def Java(app_name, file_name_exe, download_link):
    app_name = 'java'
    try:
        # Check app is installed
        result = check_app_existed(app_name)
        if result:
            return True
        # tao folder luu file tai ve
        download_directory = f'{get_download_folder()}\{app_name}'
        make_folder_log(download_directory)
        # Download and execute install file
        download_result = download_app(download_directory, download_link)

        # If download and execute fail -> return fail
        if not download_result:
            return download_result
        # Get a list of files in the folder
        file_names = os.listdir(download_directory)

        # Filter out .exe files
        install_files = [file for file in file_names if
                         (file.endswith('.exe') or file.endswith('.msi')) and os.path.isfile(
                             os.path.join(download_directory, file))]
        # Ensure download_directory and file_name_exe are correctly defined earlier in your code
        install_path = f"{download_directory}\{install_files[0]}"
        print(install_path)
        # #
        # Install app with quoted paths
        subprocess.Popen(['start', install_path], shell=True)
        sleep(15)
        # click button
        try:
            app = Application(backend='uia').connect(title='Java Setup - Welcome')
            app_window = app.window(title='Java Setup - Welcome')
            app_window.set_focus()
            app_window.child_window(title='Install', control_type='Button').click_input()

            for i in range(60):
                try:
                    app = Application(backend='uia').connect(title='Java Setup - Complete')
                    app_window = app.window(title='Java Setup - Complete')
                    sleep(1)
                    app_window.set_focus()
                    install_success = app_window.child_window(title='You have successfully installed Java')
                    if install_success:
                        app_window.child_window(title="Close", control_type="Button", found_index=1).click_input()
                        sleep(1)
                        break
                except Exception as e:
                    pass

        except Exception as e:
            print(e)
        # Check app install
        for i in range(60):
            result = check_app_existed(app_name)
            if result:
                print('cai dat thanh cong')
                return True
            sleep(10)

    except Exception as e:
        print(f'error install: {e}')
        return False


# if __name__ == '__main__':
#     download_link = 'https://javadl.oracle.com/webapps/download/AutoDL?BundleId=251656_7ed26d28139143f38c58992680c214a5'
#     Java(app_name, '', download_link)

    # "C:\Users\ameri\Downloads\Autodesk\DWG TrueView 2025 - English - (EN)\Setup.exe" / silent / install / norestart
